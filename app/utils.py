...
def registrar_entrada_saida(id):
    ...
    cursor.execute("""
        ...
        WHERE id = ? ORDER BY rowid DESC LIMIT 1
    """, (id,))
    row = cursor.fetchone()
    ...
    if entrada_str and not saida_str:
        ...
        if (agora - entrada_dt).total_seconds() > 30:
            cursor.execute("""
                UPDATE registros SET saida = ? 
                WHERE id = ? AND entrada = ?
            """, (agora_str, id, entrada_str))
            conn.commit()
            return "Saída"
        else:
            return "Ignorado"
        ...
        saida_dt = datetime.strptime(saida_str, "%Y-%m-%d %H:%M:%S")
        if (agora - saida_dt).total_seconds() > 30:
            cursor.execute("INSERT INTO registros (id, entrada) VALUES (?, ?)", (id, agora_str))
            conn.commit()
            return "Entrada"
